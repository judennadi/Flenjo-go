...
import { REMOVE_USER, TOGGLE_LOGIN, TOGGLE_REG } from "../../reducers/auth";
...
const Nav = ({ location, navigate }) => {
  ...
  const { user, isAuth, isLogin, isReg } = useSelector((state) => state.auth);
  ...
  const handleModal = (e) => {
    ...
    e.currentTarget.childNodes[len - 1].classList.toggle("dep");
  };
  ...
  return (
    <>
      ...
      <nav>
        <div className="logo">
          <h1 style={{ letterSpacing: "-1px" }} onClick={() => window.location.assign("/")}>
            FLENJO
          </h1>
        </div>
        <div
          className="nav-auth-sm-con"
          style={{
            marginLeft: "auto",
            position: "relative",
        ...
        >
          <div className="nav-auth-sm" style={isAuth ? { background: "purple" } : null}>
            {!isAuth ? (
              <span onClick={() => dispatch(TOGGLE_LOGIN())}>
        ...
        <div className="nav-auth-xl">
          {!isAuth ? (
            <>
              <button className="login" onClick={() => dispatch(TOGGLE_LOGIN())}>
                Log in
              </button>
              <button className="signup" onClick={() => dispatch(TOGGLE_REG())}>
                Sign up
              </button>
            </>
          ) : (
